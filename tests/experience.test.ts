...
describe('training XP progression', () => {
  ...
  test('maps rank-up feed activities without accepting malformed payloads', async () => {
    rpc.mockResolvedValueOnce({ data: { activities: [{ id: 'rank-1', kind: 'rank_up', author_alias: 'Bro', author_avatar_id: 'capybara-athlete', author_theme_id: 'blue', payload: { level: 5, rank: 'Intermedio' }, created_at: '2026-08-02T00:00:00Z' }, { id: 'bad', kind: 'rank_up', payload: {} }], next_cursor: 'next' }, error: null });
    await expect(getCommunityActivities()).resolves.toEqual(expect.objectContaining({ activities: [expect.objectContaining({ rank: 'Intermedio', authorAlias: 'Bro' })], nextCursor: 'next' }));
