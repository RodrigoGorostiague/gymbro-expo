...
describe('SocialProfileCard', () => {
  test('makes the complete card an accessible detail action and preserves the relationship badge', async () => {
    ...
    await act(async () => { tree = TestRenderer.create(React.createElement(SocialProfileCard, { profile: { uid: 'member-2', alias: 'Alex', avatarId: 'capigirl', categories: { trainingStyle: 'Fuerza' }, presentationThemeId: null, relationshipStatus: 'partner' }, onPress })); });
    ...
    expect(tree!.root.findAll((node) => String(node.type) === 'Text').map((node) => node.children.join(''))).toEqual(expect.arrayContaining(['Alex', 'Partner', 'Fuerza']));
  ...
  test('uses the connected athlete presentation theme with rank and a mini radar', async () => {
    let tree: TestRenderer.ReactTestRenderer | undefined;
    await act(async () => { tree = TestRenderer.create(React.createElement(SocialProfileCard, { profile: { uid: 'member-2', alias: 'Moon athlete', avatarId: 'capigirl', categories: {}, presentationThemeId: 'moon', relationshipStatus: 'bro' }, insights: { progress: { level: 12, rank: 'Avanzado' }, muscleDistribution: [{ id: 'chest', label: 'Pecho', value: 3 }] }, onPress: vi.fn() })); });
