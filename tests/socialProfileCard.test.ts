...
describe('SocialProfileCard', () => {
  test('makes the complete card an accessible detail action and preserves the relationship badge', async () => {
    ...
    expect(tree!.root.findAll((node) => String(node.type) === 'Text').map((node) => node.children.join(''))).toEqual(expect.arrayContaining(['Alex', 'GymCrush']));
    expect(tree!.root.find((node) => node.props.accessibilityLabel === 'Título Forjando Base')).toBeDefined();
    await act(async () => { card.props.onPress(); });
    expect(onPress).toHaveBeenCalledOnce();
  });

  test('uses the connected athlete presentation theme with rank and a body map', async () => {
    let tree: TestRenderer.ReactTestRenderer | undefined;
    ...
    const map = tree!.root.findByType(MuscleBodyMap);
    expect(map.props.palette.primary).toBe('#9FA8DA');
    expect(map.props.projection.entries.find((e:any)=>e.id==='chest').value).toBe(3);
  });
});
