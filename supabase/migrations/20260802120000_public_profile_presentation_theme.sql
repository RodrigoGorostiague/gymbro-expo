-- Presentation theme is explicitly public metadata. Private shop inventory/equipment remains private.
...
set presentation_theme_id = profile.presentation_theme_id
from public.profiles as profile
where profile.id = public_profile.id
  and public_profile.presentation_theme_id is distinct from profile.presentation_theme_id;
...
begin
  ...
  insert into public.public_profiles (id, alias, normalized_alias, categories, avatar_id, presentation_theme_id, updated_at)
  values (new.id, new.alias, new.normalized_alias, visible_categories, new.avatar_id, new.presentation_theme_id, now())
...
after insert or update of alias, categories, category_visibility, avatar_id, presentation_theme_id or delete on public.profiles
...
begin
  ...
  for fetched in
    select candidate.id, candidate.alias, candidate.categories, candidate.avatar_id, candidate.presentation_theme_id, candidate.directory_bucket, candidate.directory_rank
  ...
  loop
    ...
    if row_count <= bounded_size then
      profiles := profiles || jsonb_build_object('id', fetched.id, 'alias', fetched.alias, 'categories', fetched.categories, 'avatar_id', fetched.avatar_id, 'presentation_theme_id', fetched.presentation_theme_id);
...
begin
  ...
  for fetched in
    select candidate.id, candidate.alias, candidate.categories, candidate.avatar_id, candidate.presentation_theme_id, candidate.normalized_alias,
  ...
  loop
    row_count := row_count + 1;
    if row_count <= bounded_size then
      profiles := profiles || jsonb_build_object('id', fetched.id, 'alias', fetched.alias, 'categories', fetched.categories, 'avatar_id', fetched.avatar_id, 'presentation_theme_id', fetched.presentation_theme_id, 'relationship_status', fetched.relationship_status, 'requested_kind', fetched.requested_kind);
