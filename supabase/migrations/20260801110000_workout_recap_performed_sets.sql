-- Performed sets are part of an immutable recap, not an optional template feature.
      ...
      where jsonb_typeof(exercise.value) <> 'object'
        ...
        or exists (select 1 from jsonb_array_elements(exercise.value -> 'sets') set_value(value)
          where jsonb_typeof(set_value.value) <> 'object' or not set_value.value ?& array['weight', 'reps', 'completed']
            or exists (select 1 from jsonb_object_keys(set_value.value) key where key <> all(array['weight', 'reps', 'completed']))
            ...
            or jsonb_typeof(set_value.value -> 'completed') <> 'boolean')) then
