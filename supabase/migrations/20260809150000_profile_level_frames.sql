-- Equipped frames are public presentation metadata, but eligibility is always server-owned.
alter table public.profiles
  add column equipped_frame_id text not null default 'principiante'
  check (equipped_frame_id in ('principiante', 'intermedio', 'avanzado', 'gymbro', 'gymrat', 'g-boom', 'alfa', 'sigma'));
...
set equipped_frame_id = profile.equipped_frame_id
from public.profiles as profile
where profile.id = public_profile.id
  and public_profile.equipped_frame_id is distinct from profile.equipped_frame_id;

create function public.profile_frame_unlock_level(frame_id_input text)
returns integer
language sql
immutable
set search_path = ''
...
begin
  ...
  insert into public.public_profiles (id, alias, normalized_alias, categories, avatar_id, presentation_theme_id, equipped_frame_id, updated_at)
  values (new.id, new.alias, new.normalized_alias, visible_categories, new.avatar_id, new.presentation_theme_id, new.equipped_frame_id, now())
...
after insert or update of alias, categories, category_visibility, avatar_id, presentation_theme_id, equipped_frame_id or delete on public.profiles
...
begin
  select jsonb_build_object(
    'id', id, 'alias', alias, 'avatar_id', avatar_id, 'equipped_frame_id', equipped_frame_id,
    'categories', categories, 'category_visibility', category_visibility,
...
set search_path = ''
as $$
declare actor uuid := public.require_actor(); alias_input text; frame_id_input text; current_level integer;
begin
  if profile_input is null or jsonb_typeof(profile_input) <> 'object'
    or not profile_input ?& array['alias', 'avatar_id', 'equipped_frame_id', 'categories', 'category_visibility', 'auto_share_completed_workouts', 'share_routine_template', 'share_mesocycle_template', 'share_performed_set_details', 'share_social_activity', 'share_social_progress', 'share_social_consistency', 'share_social_statistics', 'share_social_muscle_distribution']
    or exists (select 1 from jsonb_object_keys(profile_input) as keys(value) where keys.value <> all(array['alias', 'avatar_id', 'equipped_frame_id', 'categories', 'category_visibility', 'auto_share_completed_workouts', 'share_routine_template', 'share_mesocycle_template', 'share_performed_set_details', 'share_social_activity', 'share_social_progress', 'share_social_consistency', 'share_social_statistics', 'share_social_muscle_distribution']))
    or jsonb_typeof(profile_input -> 'alias') <> 'string'
    or jsonb_typeof(profile_input -> 'avatar_id') <> 'string'
    or jsonb_typeof(profile_input -> 'equipped_frame_id') <> 'string'
  ...
  frame_id_input := profile_input ->> 'equipped_frame_id';
  ...
  if public.profile_frame_unlock_level(frame_id_input) is null or current_level < public.profile_frame_unlock_level(frame_id_input) then
    raise exception 'profile frame is not unlocked';
  end if;

  perform private.ensure_actor_profile(actor);
  update public.profiles set
    alias = alias_input, avatar_id = profile_input ->> 'avatar_id', equipped_frame_id = frame_id_input,
    categories = profile_input -> 'categories', category_visibility = profile_input -> 'category_visibility',
...
declare
  ...
  decoded jsonb; cursor_bucket smallint; cursor_rank uuid; cursor_id uuid; fetched record;
  profiles jsonb := '[]'::jsonb; row_count integer := 0; last_bucket smallint; last_rank uuid; last_id uuid;
begin
  if cursor is not null then
    begin
      decoded := convert_from(decode(cursor, 'base64'), 'UTF8')::jsonb;
      cursor_bucket := (decoded ->> 'b')::smallint; cursor_rank := (decoded ->> 'r')::uuid; cursor_id := (decoded ->> 'u')::uuid;
  ...
  for fetched in
    select candidate.id, candidate.alias, candidate.categories, candidate.avatar_id, candidate.equipped_frame_id, candidate.presentation_theme_id, candidate.directory_bucket, candidate.directory_rank
    ...
    order by candidate.directory_bucket, candidate.directory_rank, candidate.id limit bounded_size + 1
  loop
    row_count := row_count + 1;
    if row_count <= bounded_size then
      profiles := profiles || jsonb_build_object('id', fetched.id, 'alias', fetched.alias, 'categories', fetched.categories, 'avatar_id', fetched.avatar_id, 'equipped_frame_id', fetched.equipped_frame_id, 'presentation_theme_id', fetched.presentation_theme_id);
...
begin
  ...
  for fetched in
    select candidate.id, candidate.alias, candidate.categories, candidate.avatar_id, candidate.equipped_frame_id, candidate.presentation_theme_id, candidate.normalized_alias,
  ...
  loop
    row_count := row_count + 1;
    if row_count <= bounded_size then
      profiles := profiles || jsonb_build_object('id', fetched.id, 'alias', fetched.alias, 'categories', fetched.categories, 'avatar_id', fetched.avatar_id, 'equipped_frame_id', fetched.equipped_frame_id, 'presentation_theme_id', fetched.presentation_theme_id, 'relationship_status', fetched.relationship_status, 'requested_kind', fetched.requested_kind);
...
declare
  actor uuid := public.require_actor(); bounded_size integer := least(greatest(coalesce(page_size, 20), 1), 50);
  decoded jsonb; cursor_alias text; cursor_id uuid; fetched record;
  profiles jsonb := '[]'::jsonb; row_count integer := 0; last_alias text; last_id uuid;
begin
  if cursor is not null then
    begin
  ...
  for fetched in
    select profile.id, profile.alias, profile.categories, profile.avatar_id, profile.equipped_frame_id, profile.normalized_alias
    ...
    order by profile.normalized_alias, profile.id limit bounded_size + 1
  loop
    row_count := row_count + 1;
    if row_count <= bounded_size then
      profiles := profiles || jsonb_build_object('id', fetched.id, 'alias', fetched.alias, 'categories', fetched.categories, 'avatar_id', fetched.avatar_id, 'equipped_frame_id', fetched.equipped_frame_id);
      last_alias := fetched.normalized_alias; last_id := fetched.id;
    end if;
  end loop;
  return jsonb_build_object('profiles', profiles, 'next_cursor', case when row_count > bounded_size then encode(convert_to(jsonb_build_object('a', last_alias, 'u', last_id)::text, 'UTF8'), 'base64') else null end);
