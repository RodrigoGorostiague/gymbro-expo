...
insert into public.training_states (owner_id, attempts) values ('60000000-0000-0000-0000-000000000002', '[{"id":"attempt-1","version":1,"owner":"60000000-0000-0000-0000-000000000002","recordedRoutineName":"Upper","completedAt":"2026-08-01T10:00:00Z","durationSeconds":60,"restTimerSeconds":0,"completion":{"validSets":1,"plannedSets":1,"adherence":1,"displayPercent":100,"status":"fully-completed"},"reward":{"setGems":0,"completionGems":0,"fullCompletionBonus":0,"totalGems":0,"qualifiesForCompletion":false},"rewardApplication":{"id":"receipt-1","state":"pending"},"exercises":[{"exerciseId":"EX-0001","recordedName":"Press","catalog":{"muscleParticipations":[{"muscleGroupId":"GM-101","role":"Principal","relevance":1}]},"sets":[{"plan":{"id":"s","type":1},"result":{"setId":"s","performed":true,"performance":{"mode":"external-load","unit":"kg","reps":8,"load":20}}}]}]},{"id":"attempt-2","version":1,"owner":"60000000-0000-0000-0000-000000000002","recordedRoutineName":"Upper","completedAt":"2026-08-02T10:00:00Z","durationSeconds":60,"restTimerSeconds":0,"completion":{"validSets":1,"plannedSets":1,"adherence":1,"displayPercent":100,"status":"fully-completed"},"reward":{"setGems":0,"completionGems":0,"fullCompletionBonus":0,"totalGems":0,"qualifiesForCompletion":false},"rewardApplication":{"id":"receipt-2","state":"pending"},"exercises":[{"exerciseId":"EX-0001","recordedName":"Press","catalog":{"muscleParticipations":[{"muscleGroupId":"GM-101","role":"Principal","relevance":1}]},"sets":[{"plan":{"id":"s","type":1},"result":{"setId":"s","performed":true,"performance":{"mode":"external-load","unit":"kg","reps":8,"load":20}}}]}]}]'::jsonb);

update public.training_states set attempts = (select jsonb_agg(jsonb_set(value, '{completedAt}', to_jsonb((now()-interval '1 day')::text))) from jsonb_array_elements(attempts)) where owner_id='60000000-0000-0000-0000-000000000002';

set local role authenticated;
select set_config('request.jwt.claim.sub', '60000000-0000-0000-0000-000000000001', true);
select is((select (entry.value ->> 'value')::numeric from jsonb_array_elements(public.get_social_profile_insights('60000000-0000-0000-0000-000000000002') -> 'muscle_distribution') entry(value) where entry.value ->> 'id' = 'chest'), .5::numeric, 'accepted connection receives weekly equivalent sets from repeated attempts');
select is((public.get_social_profile_insights('60000000-0000-0000-0000-000000000002') -> 'statistics' ->> 'completed_exercises_last_90_days')::integer, 2, 'safe statistics expose aggregate counts only');
