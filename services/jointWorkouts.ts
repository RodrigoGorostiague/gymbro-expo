...
export type JointCompletedWorkout = { routineName: string; durationSeconds: number; exercises: Array<{ name: string; muscleGroupIds: string[]; sets: Array<{ weight: number; reps: number; completed: boolean }> }>; sharePayload?: WorkoutRecapSharePayload };
export type JointParticipant = { id: string; alias: string; avatarId: string; frameId?: string; titleId?: string; status: JointParticipantStatus; relationshipKind?: 'bro' | 'partner'; visibility?: JointVisibility; themeId?: string | null; isSelf?: boolean; liveProgress?: JointWorkoutLiveProgress; workout?: JointCompletedWorkout | null; sharePayload?: WorkoutRecapSharePayload | null; canInviteBro?: boolean; reactionCount?: number; commentCount?: number; viewerHasReacted?: boolean };
...
function routineSnapshot(routine: Routine): JointRoutine {
  return { name: routine.name.trim(), muscleGroups: [...new Set(routine.muscleGroups)].sort(), exercises: routine.exercises.map((exercise) => {
    ...
    return { name: exercise.name.trim(), muscleGroups: [...new Set(exercise.muscleGroups)].sort(), loadMode: exercise.loadMode ?? exercise.definitionSnapshot?.loadMode ?? 'external-load', loadUnit: exercise.loadUnit ?? exercise.definitionSnapshot?.loadUnit ?? 'kg', variant: exercise.variant, sets: exercise.sets.map(({ tipo, weight, reps, effortTarget, backoffGroupId }) => ({ tipo, weight, reps, ...(effortTarget ? { effortTarget } : {}), ...(backoffGroupId ? { backoffGroup: groups.get(backoffGroupId) ?? (groups.set(backoffGroupId, groups.size), groups.size - 1) } : {}) })) };
  }) };
}
...
export function completedJointWorkoutInput(routine: Routine, durationSeconds: number, exercises: readonly CompletedExercise[], sharePayload?: WorkoutRecapSharePayload): JointCompletedWorkout {
  if (!routine.name.trim() || !Number.isInteger(durationSeconds) || durationSeconds < 0 || durationSeconds > 18_000) throw new Error('This completed workout cannot be shared.');
  return { routineName: routine.name.trim(), durationSeconds, exercises: exercises.map((exercise) => ({ name: exercise.name.trim(), muscleGroupIds: [...new Set(exercise.muscleGroupIds ?? [])].sort(), sets: exercise.sets.map(({ weight, reps, completed }) => ({ weight, reps, completed })) })), ...(sharePayload ? { sharePayload } : {}) };
}
function asCompletedWorkout(value: unknown): JointCompletedWorkout | null { if (!value || typeof value !== 'object') return null; const row = value as Record<string, unknown>; if (typeof row.routineName !== 'string' || typeof row.durationSeconds !== 'number' || !Array.isArray(row.exercises)) return null; return { routineName: row.routineName, durationSeconds: row.durationSeconds, exercises: row.exercises.flatMap((exercise) => { if (!exercise || typeof exercise !== 'object') return []; const item = exercise as Record<string, unknown>; if (typeof item.name !== 'string' || !Array.isArray(item.muscleGroupIds) || !Array.isArray(item.sets)) return []; return [{ name: item.name, muscleGroupIds: item.muscleGroupIds.filter((muscle): muscle is string => typeof muscle === 'string'), sets: item.sets.flatMap((set) => { if (!set || typeof set !== 'object') return []; const value = set as Record<string, unknown>; return typeof value.weight === 'number' && typeof value.reps === 'number' && typeof value.completed === 'boolean' ? [{ weight: value.weight, reps: value.reps, completed: value.completed }] : []; }) }]; }) }; }
...
export async function listJointWorkoutPosts(): Promise<Array<{ id: string; createdAt: string; participants: JointParticipant[] }>> { const { data, error } = await client().rpc('list_joint_workout_posts'); if (error) throw new Error(error.message); return Array.isArray(data) ? data.flatMap((row) => { if (!row || typeof row !== 'object') return []; const value = row as Record<string, unknown>; if (typeof value.id !== 'string' || typeof value.created_at !== 'string') return []; return [{ id: value.id, createdAt: value.created_at, participants: Array.isArray(value.participants) ? value.participants.flatMap((participant) => { const parsed = asParticipant(participant); return parsed ? [parsed] : []; }) : [] }]; }) : []; }
...
export async function getJointParticipantPublicationDetail(workoutId: string, participantId: string): Promise<WorkoutRecapDetail | null> {
  ...
  return {
    ...
    metrics: { volume: participant.workout.exercises.reduce((total, exercise) => total + exercise.sets.reduce((sum, set) => sum + (set.completed ? set.weight * set.reps : 0), 0), 0) },
    caption: null,
    createdAt,
