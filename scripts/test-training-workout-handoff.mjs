...
const database = `gymbro_handoff_test_${randomUUID().replaceAll('-', '')}`;
...
try {
  ...
  assert.ok((await client.query("select to_regprocedure('public.sync_offline_workout(jsonb,jsonb,jsonb)') as rpc")).rows[0].rpc, 'Existing offline RPC prerequisite required');
  if (!(await client.query("select to_regprocedure('public.start_training_workout(jsonb)') as rpc")).rows[0].rpc) {
    await client.query(readFileSync(new URL('../supabase/migrations/20260910190000_training_workout_handoff.sql', import.meta.url), 'utf8'));
  }
  await client.query('create extension if not exists pgtap with schema extensions; set search_path = public, extensions');
  for (const [file, expected] of [['training_workout_handoff.sql', 23], ['offline_workout.sql', 19], ['joint_workouts.sql', 75], ['joint_workout_recovery.sql', 14]]) {
    ...
    assert.equal(tap.filter((line) => /^ok [0-9]+/m.test(line)).length, expected);
    console.log(`PASS: ${file}: ${expected} SQL assertions; transaction rolled back.`);
  }
  ...
  const base = {version:1,owner,attemptId:'concurrent',routineId:'r',startedAtMs:1,restTimerSeconds:30,completedSets:{},setValues:{},routineSnapshot:{id:'r',name:'Concurrent',exercises:[]}};
  ...
  const starts = await Promise.all([a,b].map((c,i) => c.query('select public.start_training_workout($1) as result', [{...base, attemptId:`concurrent-${i}`}])));
  assert.deepEqual(starts.map((r) => r.rows[0].result.status).sort(), ['existing','started']);
  assert.deepEqual(starts[0].rows[0].result.draft, starts[1].rows[0].result.draft);
  Object.assign(base, starts[0].rows[0].result.draft);
  console.log('PASS: concurrent first-ever starts share one canonical attempt.');
