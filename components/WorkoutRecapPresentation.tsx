...
export function WorkoutRecapPresentation({ recap, copyState = null, copiedKinds = [], onCopyRoutine, onCopyMesocycle, onViewMesocycle }: Props) {
  ...
  return <>
    ...
    <RecapBodyMap exercises={recap.exercises} />
    ...
    {recap.exercises.length ? <GlassCard style={styles.card}><Text style={[styles.section, { color: theme.text }]}>Resultados reales</Text><Text style={{ color: theme.textMuted }}>Valores registrados durante la sesión</Text>{recap.exercises.map((exercise, exerciseIndex) => <View key={`${exercise.name}-${exerciseIndex}`} style={[styles.performedExercise, { borderColor: theme.glassBorder }]}><Text style={[styles.exerciseName, { color: theme.text }]}>{exerciseIndex + 1}. {exercise.name}</Text><View style={styles.sets}>{exercise.sets.map((set, setIndex) => <View key={setIndex} style={[styles.setRow, { backgroundColor: set.completed ? `${theme.primary}18` : 'transparent', borderColor: set.completed ? theme.primary : theme.glassBorder }]}><Text style={[styles.setNumber, { color: theme.textMuted }]}>S{setIndex + 1}</Text><Text style={[styles.setValue, { color: theme.text }]}>{set.durationSeconds !== undefined ? `${set.durationSeconds} s` : set.weight === 0 ? `${set.reps} reps` : `${set.weight} kg × ${set.reps}`}</Text><Text style={[styles.setState, { color: set.completed ? theme.primary : theme.textMuted }]}>{set.completed ? 'Hecha' : 'Pendiente'}</Text>{set.actualEffort ? <Text style={{ color: theme.primary }}>{set.actualEffort.kind.toUpperCase()} {set.actualEffort.value}</Text> : null}</View>)}</View></View>)}</GlassCard> : null}
