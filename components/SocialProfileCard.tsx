import { muscleVolumeScale } from '../utils/muscleVolume';
...
import { projectVolumeBody, projectSharedBody } from '../utils/bodyMapProjection';
...
export function SocialProfileCard({ profile, insights, onPress }: { profile: PublicProfile; insights?: SocialProfileInsights; onPress: () => void }) {
  ...
  const content = <><ProfileAvatar avatarId={profile.avatarId} frameId={profile.frameId} level={insights?.progress?.level} size={56} borderColor={connected ? '#FFFFFF' : '#A1A1AA'} /><View style={styles.copy}><View style={styles.identity}><Text numberOfLines={1} style={[styles.alias, { color: connected ? '#FFFFFF' : theme.text }]}>{profile.alias}</Text>{relationship ? <Text style={[styles.badge, { backgroundColor: connected ? 'rgba(0,0,0,0.2)' : '#52525B', color: '#FFFFFF' }]}>{relationship}</Text> : null}</View><ProfileTitleBadge titleId={profile.titleId} />{connected && insights?.progress ? <Text style={styles.rank}>Nivel {insights.progress.level} · {insights.progress.rank}</Text> : <Text numberOfLines={1} style={{ color: connected ? 'rgba(255,255,255,0.84)' : theme.textMuted }}>{categories || 'Perfil público'}</Text>}</View><Text accessibilityElementsHidden style={[styles.chevron, { color: connected ? '#FFFFFF' : '#A1A1AA' }]}>›</Text></>;
  return <View style={{gap:12}}><HapticPressable accessibilityRole="button" accessibilityLabel={`Ver perfil de ${profile.alias}`} accessibilityHint="Abre el detalle del atleta" onPress={onPress} style={styles.pressable}>{connected ? <LinearGradient colors={[palette.primary, palette.accent, palette.secondary]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.card}>{content}</LinearGradient> : <View style={[styles.card, styles.disconnected]}>{content}</View>}</HapticPressable>{connected && insights?.muscleDistribution ? <MuscleBodyMap projection={insights.muscleVolume ? projectVolumeBody(insights.muscleVolume) : projectSharedBody(insights.muscleDistribution, [], 'Contribuciones por ejercicio')} scaleMax={insights.muscleVolume ? muscleVolumeScale(insights.muscleVolume) : undefined} palette={palette} /> : null}</View>;
}
