...
import { stageWorkoutCompletion } from '../services/workoutCompletionPreview';
...
import { flushPendingJointWorkoutPublications, loadJointPublicationProgress, JointPublicationProgress } from '../services/jointWorkoutPublicationQueue';
...
type SocialContextValue = {
  ...
  jointPublicationProgress: readonly JointPublicationProgress[];
  retryJointPublications: () => Promise<void>;
  jointPublicationSyncError: string | null;
...
export function SocialProvider({ children }: { children: React.ReactNode }) {
  ...
  const [jointPublicationProgress, setJointPublicationProgress] = useState<JointPublicationProgress[]>([]);
  const [jointPublicationSyncError, setJointPublicationSyncError] = useState<string | null>(null);
  ...
  const retryJointPublications = useCallback(async () => {
    if (!user) return;
    try {
      const restored = await loadJointPublicationProgress(user);
      if (account.current !== user) return;
      setJointPublicationProgress(restored);
      const published = await flushPendingJointWorkoutPublications(user, undefined, attempts);
      const progress = await loadJointPublicationProgress(user);
      if (account.current !== user) return;
      setJointPublicationProgress(progress);
      setJointPublicationSyncError(null);
      if (published) setRealtimeRevision((revision) => revision + 1);
    } catch (error) {
      if (account.current === user) setJointPublicationSyncError(error instanceof Error ? error.message : 'No se pudo sincronizar la publicación conjunta.');
    }
  }, [attempts, user]);
  useEffect(() => {
    setJointPublicationProgress([]);
    setJointPublicationSyncError(null);
  }, [user]);
  useEffect(() => {
    if (!user) return;
    void retryJointPublications();
    const interval = setInterval(() => void retryJointPublications(), 30_000);
    return () => clearInterval(interval);
  }, [retryJointPublications, user]);
  useEffect(() => {
    ...
    for (const attempt of attempts) {
      ...
      void stageWorkoutCompletion(attempt.id, input, publicationKey).then(async (reviewManaged) => {
        if (!reviewManaged) await createWorkoutRecap(input, publicationKey);
        return [];
      }).catch((error) => {
  ...
  const value = useMemo(() => ({ jointPublicationProgress, retryJointPublications, jointPublicationSyncError, ownProfile, refreshOwnProfile, saveProfile, discover: getDiscoveryPage, search: searchProfiles, circle: getCirclePage, requests: getRequestPage, blockedUsers: getBlockedUsersPage, getProfile: getPublicProfile, getSummary: getGraphSummary, getProfileInsights: getSocialProfileInsights, getProfileInsightsBatch: getSocialProfileInsightsBatch, command: runGraphCommand, getWorkoutRecaps: getWorkoutRecapPage, getWorkoutRecapDetail, getCommunityActivities, createWorkoutRecap, deleteWorkoutRecap, failedAutoRecapSessionIds, clearFailedAutoRecapSession, getProfilePlanLibrary, createPrivatePlanShareRequest, listReceivedPrivatePlanShareRequests, acceptPrivatePlanShareRequest, rejectPrivatePlanShareRequest, realtimeRevision }), [jointPublicationProgress, retryJointPublications, jointPublicationSyncError, ownProfile, realtimeRevision, refreshOwnProfile, saveProfile, failedAutoRecapSessionIds, clearFailedAutoRecapSession]);
