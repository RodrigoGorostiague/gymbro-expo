...
import { MUSCLE_GROUP_OPTIONS, MUSCLE_GROUP_LABELS } from '../../constants/muscleGroups';
...
export default function ProgressScreen() {
  ...
  const { user } = useAuth();
  const { exercises, routines, sessions, attempts, quarantinedSessionCount, resolveQuarantine, dataState, dataError, retryData } = useData();
      ...
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          ...
          : <GlassCard style={styles.chartCard}>
            ...
            {scope === 'muscle' ? <View><Text style={[styles.chartTitle, { color: theme.text }]}>Exposición muscular ponderada</Text><Text style={[styles.body, { color: theme.textMuted }]}>Exposición registrada ponderada; no representa crecimiento muscular.</Text><View style={styles.wrap}>{MUSCLE_GROUP_OPTIONS.map((item) => <FilterChip key={item.value} label={item.label} selected={muscle === item.value} onPress={() => setMuscle(item.value)} />)}</View><Text style={[styles.detail, { color: theme.text }]}>{MUSCLE_GROUP_LABELS[muscle]}: {(exposure.current[muscle] ?? 0).toFixed(1)} actuales · {(exposure.previous[muscle] ?? 0).toFixed(1)} anteriores</Text></View> : null}
