...
export default function MesocyclesScreen({ navigation }: { navigation?: React.ReactNode }) {
  ...
  return <ThemeBackground><SafeAreaView style={styles.safe}><AppScreenHeader title="Mesociclos" subtitle="Bloques para planificar semanas de entrenamiento" trailing={<GlassButton title="+ Nuevo" onPress={() => router.push('/mesocycle/create')} />}/>{mesocycles.length === 0 ? <ScrollView contentContainerStyle={styles.list}>{navigation}<TrainingHelp topic="mesocycles" /><GlassCard style={styles.emptyCard}><Text style={[styles.emptyTitle, { color: theme.text }]}>Todavía no tienes mesociclos</Text><Text style={[styles.emptyText, { color: theme.textMuted }]}>Crea un bloque para organizar objetivos, duración y la estructura semanal antes de planificar sesiones.</Text><View style={styles.emptyAction}><GlassButton title="Entrenar con una rutina" variant="secondary" onPress={() => router.push('/(tabs)/routines')} /><GlassButton title="Crear mi primer mesociclo" onPress={() => router.push('/mesocycle/create')} /></View></GlassCard></ScrollView> : <FlatList ListHeaderComponent={<>{navigation}<TrainingHelp topic="mesocycles" /></>} data={sections} keyExtractor={(section) => section.key} contentContainerStyle={styles.list} renderItem={({ item: section }) => {
