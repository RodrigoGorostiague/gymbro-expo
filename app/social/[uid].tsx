...
export default function PublicProfileScreen() {
  ...
  return <ThemeBackground theme={profileTheme}><SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.scroll}><AppNavBar onBack={() => router.back()} />
    {loading ? <PublicProfileSkeleton theme={theme} /> : !profile ? <Text style={{ color: theme.text }}>Este perfil no está disponible.</Text> : <>
      ...
      {summary?.blocked ? <GlassButton theme={profileTheme} title="Desbloquear" onPress={() => void act({ command: 'unblock', targetId: uid })} loading={acting} /> : summary?.incomingRequest ? <><GlassButton theme={profileTheme} title={`Aceptar solicitud de ${requestKindLabel}`} onPress={() => void act({ command: 'respondRequest', targetId: uid, accepted: true })} loading={acting} /><GlassButton theme={profileTheme} title="Rechazar solicitud" variant="secondary" onPress={() => void act({ command: 'respondRequest', targetId: uid, accepted: false })} disabled={acting} /></> : summary?.outgoingRequest ? <GlassButton theme={profileTheme} title="Cancelar solicitud" onPress={() => void act({ command: 'cancelRequest', targetId: uid })} loading={acting} /> : summary?.relationshipKind === 'bro' ? <GlassButton theme={profileTheme} title="Solicitar upgrade a Partner" onPress={() => void act({ command: 'sendRequest', targetId: uid, relationshipKind: 'partner' })} loading={acting} /> : summary?.relationshipKind === 'partner' ? <GlassButton theme={profileTheme} title="Bajar a Bro" variant="secondary" onPress={downgrade} disabled={acting} /> : <><GlassButton theme={profileTheme} title="Invitar como Bro" onPress={() => void act({ command: 'sendRequest', targetId: uid, relationshipKind: 'bro' })} loading={acting} /><GlassButton theme={profileTheme} title="Invitar como Partner" variant="secondary" onPress={() => void act({ command: 'sendRequest', targetId: uid, relationshipKind: 'partner' })} disabled={acting} /></>}
    </>}
