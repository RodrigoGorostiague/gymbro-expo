...
export default function JointWorkoutScreen() {
  const { theme } = useTheme(); const { activeWorkoutDraft, prepareOnlineWorkout, associateActiveWorkoutJoint } = useData(); const { realtimeRevision } = useSocial();
  const { user } = useAuth();
