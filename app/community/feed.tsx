...
import { CommunityRankUpActivity, WorkoutRecap } from '../../types';
...
export default function CommunityFeedScreen() {
  ...
  const [rankActivities, setRankActivities] = useState<readonly CommunityRankUpActivity[]>([]);
  const load = useCallback(async (nextCursor: string | null = null, append = false) => {
    setLoading(true); setError(null);
    try {
      const [page, posts, activities] = await Promise.all([getWorkoutRecaps(nextCursor), nextCursor ? Promise.resolve(null) : listJointWorkoutPosts(), nextCursor ? Promise.resolve(null) : getCommunityActivities()]);
      ...
      if (activities) setRankActivities(activities.activities);
  ...
  const rankCard = (activity: CommunityRankUpActivity) => {
    const authorTheme = getShopTheme(activity.authorThemeId ?? '') ?? THEMES.rodaja;
    return <GlassCard key={`rank-${activity.id}`} style={styles.card}><LinearGradient colors={[authorTheme.primary, authorTheme.accent, authorTheme.secondary]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.authorBanner}><ProfileAvatar avatarId={activity.authorAvatarId} size={48} borderColor="rgba(255,255,255,0.7)" /><View style={styles.authorCopy}><Text style={styles.alias}>{activity.authorAlias}</Text><Text style={styles.authorAction}>Alcanzó un nuevo rango</Text></View><Text style={styles.badge}>RANGO</Text></LinearGradient><Text style={[styles.recapTitle, { color: theme.text }]}>Nivel {activity.level} · {activity.rank}</Text><Text style={[styles.detailLink, { color: authorTheme.primary }]}>El entrenamiento constante da resultados.</Text></GlassCard>;
  };
